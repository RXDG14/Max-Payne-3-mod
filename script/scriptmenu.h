...
const ColorRgba
	MenuItemTitle_colorRect { 255, 0, 0, 255 },
...
public:
	...
	{
		return {
			IsKeyDown(VK_RETURN) ,//|| (IsKeyDownLong(VK_CONTROL) && IsKeyDown(VK_RETURN)),
			IsKeyDown(VK_NUMPAD0) || MenuSwitchPressed() || IsKeyDown(VK_BACK),
			IsKeyDown(VK_UP) ,//|| (IsKeyDownLong(VK_CONTROL) && IsKeyDown(VK_UP)),
			IsKeyDown(VK_DOWN),// || (IsKeyDownLong(VK_CONTROL) && IsKeyDown(VK_DOWN)),
			IsKeyDown(VK_RIGHT),// || (IsKeyDownLong(VK_CONTROL) && IsKeyDown(VK_RIGHT)),
			IsKeyDown(VK_LEFT)// || (IsKeyDownLong(VK_CONTROL) && IsKeyDown(VK_LEFT))
		};
	}
